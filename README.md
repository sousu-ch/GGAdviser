...
1.  **【重要】権限の付与**:
    - 「🔐 権限とプライバシー」セクションにある **「キャプチャ権限を付与する」** ボタンを押し、権限を許可してください。
    - ※ これを行わないと、画面キャプチャ機能（分析機能の中核）が動作しません。
2.  **Result Map URL**: 
    - 自分が作成した `map-making.app` のマップURL（例: `https://map-making.app/maps/123456`）を入力して保存します。

...
2.  **地図設定リストの登録**:
    - 自分が使用したい `map-making.app` のマップURLを登録します。「タイトル自動取得」で見やすい名前を付けて、複数のマップを使い分けることが可能です。
...
### D. プロンプト設定リスト（自分専用のAIにする）
オプション画面の **プロンプト設定リスト** から、用途に合わせたプロンプトを複数作成・編集できます。

- `{{lat}}`, `{{lng}}`, `{{date}}` といった変数を使用すると、解析時に実際の地点データに自動置換されます。
- 「電柱の形を詳しく教えて」「看板の言語から国を判定して」など、指示を具体的にするほど分析の精度が向上します。
- 「全プロンプトを初期化」ボタンで、いつでも推奨の標準テンプレートに戻せます。
...
### v1.1.1 (2026-03-27)
- **[Fix] クラシックモードでのラウンド座標取得バグを修正**: 
    - 一部の画面（Gameページの結果画面等）で、今終わったラウンドではなく次のラウンドの座標が送られてしまう問題を解消しました。
    - セレクタの精度を向上させ、GeoGuessrの内部的なUI属性（`data-qa="round-number"`）に対応しました。
- **[Refactor]**: ラウンド判定ロジックを整理し、コードの堅牢性と可読性を向上させました。

### v1.1.0 (2026-03-22)
- 生産性向上とセキュリティ強化のための大規模アップデート。
- ID ベースのセキュアな通信アーキテクチャへの移行。
- UI/UX の改善と Gemini 連携の最適化。
